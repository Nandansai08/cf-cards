  ...
  return (
    <div>
      <div className="mb-1.5 grid grid-cols-[minmax(0,1fr)_auto] items-end gap-3">
        <span className="min-w-0 truncate font-display text-sm font-semibold uppercase text-muted-foreground">
          {k}
        </span>
...
function Identity({ profile, className }: { profile: PlayerProfile; className?: string }) {
  ...
  return (
    <header className={cn("border-b border-border/60 pb-5", className)}>
      <h1 className="break-words font-display text-3xl font-bold uppercase sm:text-4xl">
        {profile.handle}
      </h1>
      <p className="mt-1 text-base capitalize text-muted-foreground">
        {s.rank} <span aria-hidden>·</span>{" "}
        <span className="tabular-nums">{s.currentRating || "Unrated"} rating</span>
      </p>
        ...
        <div className="grid grid-cols-2 gap-x-5 gap-y-2 border-t border-border/60 py-3 sm:grid-cols-4">
          {secondary.map(([label, value]) => (
            <div key={label} className="flex min-w-0 items-baseline justify-between gap-2 sm:block">
              ...
              <span className="shrink-0 font-display text-sm font-semibold tabular-nums text-foreground sm:mt-0.5 sm:block">
                {value}
              </span>
            </div>
          ))}
    ...
    <section aria-labelledby="attributes-title">
      ...
      <div className="mt-4 grid gap-x-8 gap-y-5 sm:grid-cols-2">
        {ATTR_ORDER.map((key) => (
          <AttrBar
            key={key}
            k={`${key} — ${ATTR_META[key].long.split("—")[1]?.trim() ?? ""}`}
            v={profile.attrs[key]}
          />
        ))}
      </div>
      {open && (
        <div className="mt-5 border-l-2 border-primary/70 pl-4 text-sm leading-6 text-muted-foreground">
          <p>
            Each attribute uses public rating, contest, problem difficulty, tag breadth, and
            activity data. Large totals are normalized so volume does not dominate.
          </p>
          <p className="mt-3 font-medium text-foreground">
            OVR is a Codeforces Cards metric calculated deterministically from public Codeforces
            data. It is not an official Codeforces rating.
          </p>
          <p className="mt-2 font-display text-xs tabular-nums">
            OVR = {ATTR_ORDER.map((key) => `${ATTR_WEIGHTS[key].toFixed(2)}·${key}`).join(" + ")}
          </p>
        </div>
      )}
    </section>
...
function DeltaPill({ delta }: { delta: number }) {
  ...
  return (
    <span
      className={cn(
        "inline-flex items-center gap-1 font-display text-xs font-bold tabular-nums",
        delta > 0 && "text-lime",
    ...
    if (typeof navigator !== "undefined" && "share" in navigator) {
      try {
        await navigator.share({
          title: "Codeforces Cards",
          text: `${profile.handle} — OVR ${profile.ovr}`,
          url,
        });
        return;
      } catch {
      ...
      <div className="space-y-2">
        <div className="grid grid-cols-[minmax(0,1fr)_auto] overflow-hidden rounded-md">
          <Button
            ...
            disabled={busy}
            className="rounded-r-none font-display uppercase"
          >
          ...
          <label className="relative border-l border-primary-foreground/25 bg-primary text-primary-foreground">
            ...
            <select
              ...
              className="h-9 appearance-none bg-transparent py-2 pl-3 pr-8 text-xs font-semibold uppercase outline-none"
            >
              <option value="card">Front</option>
...
export function CustomizePanel({
  ...
  return (
    <details className="border-t border-border/60 pt-3 text-sm text-muted-foreground">
      <summary className="flex cursor-pointer list-none items-center gap-2 text-xs hover:text-foreground">
      ...
      <div className="mt-3 space-y-3">
        ...
        <Choice
          ...
          active={value.frame ?? "classic"}
          onPick={(frame) => onChange({ ...value, frame: frame as CardStyleOptions["frame"] })}
        />
        <Choice
          label="Background"
          options={["rays", "grid", "plain"]}
          active={value.pattern ?? "rays"}
          onPick={(pattern) =>
            onChange({ ...value, pattern: pattern as CardStyleOptions["pattern"] })
          }
        />
      </div>
    </details>
      ...
      <div className="flex flex-wrap gap-1">
        {options.map((option) => (
          <button
            ...
            className={cn(
              "border-b px-2 py-1 text-xs capitalize",
              active === option
...
export function PlayerProfileView({ profile }: { profile: PlayerProfile }) {
  ...
  const fromPeak =
    stats.currentRating && stats.maxRating ? stats.currentRating - stats.maxRating : 0;
      ...
      <div className="grid gap-8 lg:grid-cols-[360px_minmax(0,1fr)] lg:gap-14 lg:items-start">
        ...
        <main className="min-w-0 space-y-9">
          ...
          <section aria-labelledby="rating-history-title">
            <div className="grid grid-cols-[minmax(0,1fr)_auto] items-end gap-4">
              ...
              <div className="hidden items-end gap-6 sm:flex">
                {[
                  ...
                  [
                    "From peak",
                    stats.currentRating && stats.maxRating
                      ? `${fromPeak > 0 ? "+" : ""}${fromPeak}`
                      : "—",
                  ],
                ].map(([label, value]) => (
            ...
            <div className="mt-3">
              <RatingChart data={profile.history} />
            </div>
          </section>
          ...
          <Tabs defaultValue="form" id="playstyle">
            <TabsList className="flex h-auto w-full justify-start overflow-x-auto border-b border-border/60 bg-transparent p-0">
              {["form", "analysis", "achievements", "evolution"].map((tab) => (
                <TabsTrigger
                  key={tab}
                  value={tab}
                  className="rounded-none border-b-2 border-transparent px-3 py-3 font-display text-xs uppercase data-[state=active]:border-primary data-[state=active]:bg-transparent"
                >
                  {tab}
                </TabsTrigger>
              ))}
            ...
            <TabsContent value="form" className="mt-4">
              <div className="flex items-center justify-between">
                ...
                <span className="flex items-center gap-2 text-xs text-muted-foreground">
                  Last 5 <DeltaPill delta={trend} />
                </span>
              </div>
              {profile.form.length === 0 ? (
                <p className="mt-4 text-sm text-muted-foreground">No rated contests yet.</p>
              ) : (
                <ul className="mt-3 divide-y divide-border/60">
                  {profile.form.map((form) => (
                    <li
                      key={form.contestId}
                    ...
                    >
                      <div className="min-w-0">
                        ...
                        <p className="text-xs text-muted-foreground">
                          {form.date} · rank #{form.rank.toLocaleString()}
                        </p>
                      </div>
                      <div className="flex shrink-0 items-center gap-3">
                        <DeltaPill delta={form.delta} />
            ...
            <TabsContent value="analysis" className="mt-5 space-y-6">
              <div className="grid gap-6 sm:grid-cols-2">
                <div>
                  <h3 className="font-display font-semibold uppercase">Strengths</h3>
                ...
                <div>
                  <h3 className="font-display font-semibold uppercase">Development areas</h3>
                  <ul className="mt-2 space-y-2 text-sm text-muted-foreground">
              ...
              <div>
                <h3 className="font-display font-semibold uppercase">Playstyle</h3>
                <div className="mt-3 divide-y divide-border/60">
                  {profile.badges.map((badge) => (
                    ...
                    >
                      ...
                      <span className="text-right text-muted-foreground">{badge.reason}</span>
                    </div>
                  ))}
                </div>
              </div>
              <div>
                <h3 className="font-display font-semibold uppercase">Top tags</h3>
                <p className="mt-2 text-sm text-muted-foreground">
                  {stats.topTags.length
                    ? stats.topTags.map((tag) => `${tag.tag} (${tag.count})`).join(" · ")
                    : "No tagged solves yet."}
                </p>
              </div>
            </TabsContent>
            ...
            <TabsContent value="evolution" className="mt-5">
              <SectionHeading>Card evolution</SectionHeading>
              <p className="mt-1 text-sm text-muted-foreground">
                Historical OVR using data available at each year end.
              </p>
              <ol className="mt-4 divide-y divide-border/60">
                {profile.evolution.map((entry) => (
                  <li
                    key={entry.year}
                    className="grid grid-cols-[3rem_minmax(0,1fr)_3rem] items-center gap-4 py-3"
                  >
                    <span className="font-display text-sm text-muted-foreground">{entry.year}</span>
                    <div className="h-1.5 overflow-hidden rounded-full bg-secondary">
                      <div
                        className="h-full rounded-full bg-primary"
                        style={{ width: `${entry.ovr}%` }}
                      />
                    </div>
                    <span className="text-right font-display text-lg font-bold">{entry.ovr}</span>
                  </li>
                ))}
              </ol>
            </TabsContent>
          ...
          <nav
            aria-label="Related player pages"
            className="flex flex-wrap gap-x-5 gap-y-2 border-t border-border/60 pt-5 text-sm text-muted-foreground"
          >
            <Link
              to="/compare"
              search={{ a: profile.handle }}
              className="hover:text-foreground hover:underline"
            >
              Compare with a friend
            </Link>
            <Link to="/leaderboard" className="hover:text-foreground hover:underline">
              See leaderboard
            </Link>
          </nav>
        </main>
